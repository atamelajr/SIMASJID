...
class TransactionModel {
    ...
    static async getPaginatedTransactions(filters = {}, page = 1, limit = '10') {
        ...
        let sql = `
            SELECT t.*, c.name as category_name, c.sub_type, ca.name as account_name, u.full_name as created_by_name,
    ...
    /**
     * Logic Inti: Input Transaksi (dengan Tanggal Transaksi Riil)
     */
    static async createTransaction(data, userId) {
        ...
        try {
            ...
            let { account_id, category_id, type, amount, description, donor_name, proof_file, asset_item, inventory_item } = data;
            ...
            // Jika Donasi Barang / Non-Kas, account_id bisa opsional dari frontend. Gunakan akun kas default jika kosong.
            if (!account_id) {
                const [defaultAccount] = await connection.query(`SELECT id FROM cash_accounts LIMIT 1`);
                account_id = defaultAccount[0]?.id || 1;
            }

            // 1. Insert Transaction dengan Tanggal Transaksi Riil & Payment Mode Presisi
            const [trxResult] = await connection.query(
                `INSERT INTO transactions 
                 (transaction_code, transaction_date, account_id, category_id, type, payment_mode, is_in_kind, amount, description, donor_name, proof_file, created_by)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [code, transactionDate, account_id, category_id, type, paymentMode, isInKind, finalAmount, description, donor_name || null, proof_file || null, userId]
            );
            const transactionId = trxResult.insertId;

            // 2. Update Account Balance ONLY IF NOT IN-KIND
            if (isInKind === 0) {
                if (type === 'Penerimaan') {
            ...
            // Trigger Otomatis: Kategori "Modal" atau Input Aset
            ...
            // Trigger Otomatis: Kategori "Material" atau Input Persediaan
            ...
            // Log Activity
            const logMsg = isInKind === 1 
                ? `Menambahkan donasi barang (In-Kind) ${code} tgl ${transactionDate} dengan nilai Rp ${finalAmount}`
                : `Menambahkan transaksi ${type} ${code} tgl ${transactionDate} sebesar Rp ${finalAmount}`;
    ...
    static async deleteTransaction(id, userId) {
        ...
        try {
            ...
            // Reverse Cash Balance ONLY IF NOT IN-KIND
            if (trx.is_in_kind === 0) {
    ...
    static async updateTransaction(id, data, userId) {
        ...
        try {
            ...
            let { account_id, category_id, type, payment_mode, amount, description, donor_name, proof_file } = data;
            ...
            await connection.query(
                `UPDATE transactions 
                 SET transaction_date = ?, account_id = ?, category_id = ?, type = ?, payment_mode = ?, is_in_kind = ?, amount = ?, description = ?, donor_name = ?, proof_file = ?
                 WHERE id = ?`,
                [transactionDate, account_id, category_id, type, paymentMode, isInKind, finalAmount, description, donor_name || 'Hamba Allah', proof, id]
            );
